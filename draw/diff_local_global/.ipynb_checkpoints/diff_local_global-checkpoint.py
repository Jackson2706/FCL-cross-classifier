...
try:
    ...
    df_global_stamp = pd.read_csv("stamp_global.csv")
    df_local_stamp = pd.read_csv("stamp_local.csv")
...
# AF-FCL (EMA)
plt.plot(x_fcl, ema_global_fcl, label="AF-FCL Global", color="blue", linewidth=2.5)
plt.plot(x_fcl, ema_local_fcl, label="AF-FCL Local", color="blue", linestyle="--", linewidth=2.5)

# FedAvg (EMA)
plt.plot(x_fedavg, ema_global_fedavg, label="FedAvg Global", color="green", linewidth=2.5)
plt.plot(x_fedavg, ema_local_fedavg, label="FedAvg Local", color="green", linestyle="--", linewidth=2.5)

# STAMP (EMA)
plt.plot(x_stamp, ema_global_stamp, label="STAMP Global", color="red", linewidth=2.5)
plt.plot(x_stamp, ema_local_stamp, label="STAMP Local", color="red", linestyle="--", linewidth=2.5)

# Vẽ vùng STD cho AF-FCL
plt.fill_between(x_fcl, ema_global_fcl - std_global_fcl, ema_global_fcl + std_global_fcl, color="blue", alpha=0.2)
plt.fill_between(x_fcl, ema_local_fcl - std_local_fcl, ema_local_fcl + std_local_fcl, color="blue", alpha=0.2)

# Vẽ vùng STD cho FedAvg
plt.fill_between(x_fedavg, ema_global_fedavg - std_global_fedavg, ema_global_fedavg + std_global_fedavg, color="green", alpha=0.2)
plt.fill_between(x_fedavg, ema_local_fedavg - std_local_fedavg, ema_local_fedavg + std_local_fedavg, color="green", alpha=0.2)

# Vẽ vùng STD cho STAMP
plt.fill_between(x_stamp, ema_global_stamp - std_global_stamp, ema_global_stamp + std_global_stamp, color="red", alpha=0.2)
plt.fill_between(x_stamp, ema_local_stamp - std_local_stamp, ema_local_stamp + std_local_stamp, color="red", alpha=0.2)
...
if task_idx_fcl < len(ema_global_fcl) and task_idx_fcl < len(ema_local_fcl):
    ...
    plt.text(x_arrow_fcl + 5, mid_y_fcl - 0.05, gap_text_fcl, color='blue', fontsize=14, va='center')
...
if task_idx_fedavg < len(ema_global_fedavg) and task_idx_fedavg < len(ema_local_fedavg):
    ...
    plt.text(x_arrow_fedavg + 5, mid_y_fedavg + 0.1, gap_text_fedavg, color='green', fontsize=14, va='center')
...
if task_idx_stamp < len(ema_global_stamp) and task_idx_stamp < len(ema_local_stamp):
    ...
    plt.text(x_arrow_stamp + 5, mid_y_stamp + 0.1, gap_text_stamp, color='red', fontsize=14, va='center')
