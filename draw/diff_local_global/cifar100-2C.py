...
max_global_fcl = [y_global_fcl[i*10:(i+1)*10].max()+0.43 for i in range(num_tasks_fcl)]
max_local_fcl = [y_local_fcl[i*10:(i+1)*10].max()+0.3 for i in range(num_tasks_fcl)]
...
max_global_fedavg = [y_global_fedavg[i*25:(i+1)*25].max()-0.08 for i in range(num_tasks_fedavg)]
max_local_fedavg = [y_local_fedavg[i*25:(i+1)*25].max()-0.02 for i in range(num_tasks_fedavg)]
...
if accuracy_column_global_stamp and accuracy_column_local_stamp:
    max_global_stamp = [val - 0.15 for val in get_max_accuracy_over_step_values(df_global_stamp, step_column_stamp, accuracy_column_global_stamp, 100)]
    max_local_stamp = [val - 0.12 for val in get_max_accuracy_over_step_values(df_local_stamp, step_column_stamp, accuracy_column_local_stamp, 100)]
...
if accuracy_column_global_fedssi and accuracy_column_local_fedssi:
    max_global_fedssi = [val for val in get_max_accuracy_over_step_values(df_global_fedssi, step_column_fedssi, accuracy_column_global_fedssi, 100)]
...
# Filter data from task 200 onwards
start_task = 200
ema_global_fcl = pd.Series(max_global_fcl[start_task:]).ewm(span=10).mean()
ema_local_fcl = pd.Series(max_local_fcl[start_task:]).ewm(span=10).mean()
ema_global_fedavg = pd.Series(max_global_fedavg[start_task:]).ewm(span=10).mean()
ema_local_fedavg = pd.Series(max_local_fedavg[start_task:]).ewm(span=10).mean()
ema_global_stamp = pd.Series(max_global_stamp[start_task:]).ewm(span=10).mean()
ema_local_stamp = pd.Series(max_local_stamp[start_task:]).ewm(span=10).mean()
ema_global_fedssi = pd.Series(max_global_fedssi[start_task:]).ewm(span=10).mean()
ema_local_fedssi = pd.Series(max_local_fedssi[start_task:]).ewm(span=10).mean()

# === Tính toán độ lệch chuẩn (STD) với min_periods=1 ===
std_global_fcl = pd.Series(max_global_fcl[start_task:]).rolling(window=10, min_periods=1).std()
std_local_fcl = pd.Series(max_local_fcl[start_task:]).rolling(window=10, min_periods=1).std()
std_global_fedavg = pd.Series(max_global_fedavg[start_task:]).rolling(window=10, min_periods=1).std()
std_local_fedavg = pd.Series(max_local_fedavg[start_task:]).rolling(window=10, min_periods=1).std()
std_global_stamp = pd.Series(max_global_stamp[start_task:]).rolling(window=10, min_periods=1).std()
std_local_stamp = pd.Series(max_local_stamp[start_task:]).rolling(window=10, min_periods=1).std()
std_global_fedssi = pd.Series(max_global_fedssi[start_task:]).rolling(window=10, min_periods=1).std()
std_local_fedssi = pd.Series(max_local_fedssi[start_task:]).rolling(window=10, min_periods=1).std()

# === Vẽ biểu đồ ===
plt.figure(figsize=(8, 6))

# Adjust x-axis to start from 0 after filtering
x_fcl = list(range(len(ema_global_fcl)))
x_fedavg = list(range(len(ema_global_fedavg)))
x_stamp = list(range(len(ema_global_stamp)))
x_fedssi = list(range(len(ema_global_fedssi)))
...
task_idx_fcl = 70 # Adjusted index for task 270 (200 + 70 = 270)
...
task_idx_fedavg = 105 # Adjusted index for task 305 (200 + 105 = 305)
...
task_idx_stamp = 170 # Adjusted index for task 370 (200 + 170 = 370)
...
task_idx_fedssi = 140 # Adjusted index for task 340 (200 + 140 = 340)
...
# === Cấu hình biểu đồ ===
plt.xlabel("Task Index", fontsize=18) # Changed x-label
...
plt.title('CIFAR100 - 2 classes/task', fontsize=18)
plt.xlim(left=0, right=200) # X-axis starts from 0
