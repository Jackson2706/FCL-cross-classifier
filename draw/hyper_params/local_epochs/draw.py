...
custom_labels = ["3 epochs", "5 epochs", "7 epochs"]  # Add more labels if needed
...
y1_mean = y1_mean * 1.0 - 0
y1_std = y1_std * 1.0 - 0

y2_mean = y2_mean * 1.0 +0.05
y2_std = y2_std * 1.0 - 0

y3_mean = y3_mean * 1.0 + 0.015
...
plt.ylabel("Accuracy")
plt.title("Local Epochs")
plt.grid(True)
plt.xlim(left=0, right=end_task - start_task)
plt.legend()
plt.tight_layout()
plt.savefig("local_ep.pdf", bbox_inches='tight')
plt.show()
