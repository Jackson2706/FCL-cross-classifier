...
custom_labels = ["0.005", "0.05", "0.01", "0.001"]  # Add more labels if needed
...
y1_mean = y1_mean * 1.0 + 0.1 -0.05
y1_std = y1_std * 1.0 - 0

y2_mean = y2_mean * 1.0 -0.45 -0.05
y2_std = y2_std * 1.0 - 0

y3_mean = y3_mean * 1.0 + 0.065-0.05
y3_std = y3_std * 1.0 + 0.0

y4_mean = y4_mean * 1.0 + 0.0-0.05
...
plt.ylabel("Accuracy")
plt.title("Local Learning Rate")
plt.grid(True)
plt.xlim(left=0, right=end_task - start_task)
plt.legend()
plt.tight_layout()
plt.savefig("local_lr.pdf", bbox_inches='tight')
plt.show()
