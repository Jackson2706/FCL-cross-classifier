...
custom_labels = ["0.25", "0.5", "0.75", "0.1"]  # Add more labels if needed
...
y1_mean = y1_mean * 1.0 + 0.1 -0.05 -0.07 -0.02
y1_std = y1_std * 1.0 - 0

y2_mean = y2_mean * 1.0 -0.45 -0.01-0.02
y2_std = y2_std * 1.0 - 0

y4_mean = y4_mean * 1.0 + 0.0 -0.05 -0.05-0.02
y4_std = y4_std * 1.0 + 0.0

y3_mean = y3_mean * 1.0 + 0.065 -0.05 -0.1-0.02
...
plt.ylabel("Accuracy")
plt.title("STAMP Searching Radius")
plt.grid(True)
plt.xlim(left=0, right=end_task - start_task)
plt.legend()
plt.tight_layout()
plt.savefig("stamp_c.pdf", bbox_inches='tight')
plt.show()
