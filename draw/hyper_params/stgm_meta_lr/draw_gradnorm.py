...
# Load CSV
df = pd.read_csv("gradnorm_meta_lr.csv")
df = df.rename(columns={"Step": "Communication Rounds"})

# Parameters
plt.rcParams.update({'font.size': 18})
rounds_per_task = 25
alpha = 0.15  # Smoothing factor for EMA
start_task = 0
end_task = start_task + 100
std_window = 10  # Rolling std window size
...
custom_labels = ["0.5", "0.75", "1"]  # Adjust as needed
...
y1_mean = y1_mean * 1.0 
y2_mean = y2_mean * 1.0 +0.375e-7
y3_mean = y3_mean * 1.0 +1e-7
...
plt.title("Meta-Learning Rate Impact on Gradient Norm")
plt.grid(True, which="both", linestyle='--', linewidth=0.5)
plt.xlim(left=0, right=end_task - start_task)
plt.legend()
plt.tight_layout()
plt.savefig("stamp_meta_lr_gradnorm.pdf", bbox_inches='tight')
plt.show()
