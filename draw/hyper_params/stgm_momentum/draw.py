...
custom_labels = ["0.5", "0.25", "0.75", "0.001"]  # Add more labels if needed
...
y1_mean = y1_mean * 1.0 + 0.1 -0.08
y1_std = y1_std * 1.0 - 0

y2_mean = y2_mean * 1.0 -0.40 -0.05
y2_std = y2_std * 1.0 - 0
...
plt.ylabel("Accuracy")
plt.title("STAMP Momentum")
plt.grid(True)
plt.xlim(left=0, right=end_task - start_task)
plt.legend()
plt.tight_layout()
plt.savefig("stamp_momentum.pdf", bbox_inches='tight')
plt.show()
