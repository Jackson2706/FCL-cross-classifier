...
custom_labels = ["30", "50", "50", "10"]  # Add more labels if needed
...
y1_mean = y1_mean * 1.0 + 0.1 -0.15 - 0.05
y1_std = y1_std * 1.0 - 0

y2_mean = y2_mean * 1.0 -0.40 -0.1- 0.05
y2_std = y2_std * 1.0 - 0

y3_mean = y3_mean * 1.0 + 0.065-0.12- 0.05
y3_std = y3_std * 1.0 + 0.0

y4_mean = y4_mean * 1.0 + 0.0-0.15- 0.05
...
plt.ylabel("Accuracy")
plt.title("STAMP Step Size")
plt.grid(True)
plt.xlim(left=0, right=end_task - start_task)
plt.legend(loc="lower right")
plt.tight_layout()
plt.savefig("stamp_step_size.pdf", bbox_inches='tight')
plt.show()
