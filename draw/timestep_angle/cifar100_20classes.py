...
custom_labels = ['FedAvg', 'FedL2P', 'FedWeIT', 'STAMP']

# Dữ liệu
df = pd.read_csv('cifar100_20classes_t.csv')
x = df.iloc[:, 0]
...
plt.title('CIFAR100 - 20 classes/task')
plt.legend()
plt.grid(True)
plt.xlim(left=50, right=300)
plt.ylim(bottom=0.15, top=0.65)
plt.tight_layout()
plt.savefig("CIFAR100-20C.pdf", bbox_inches='tight')
plt.show()
