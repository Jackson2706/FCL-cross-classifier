...
custom_labels = ['FedAvg', 'FedL2P', 'AF-FCL', 'STAMP']

# Dữ liệu
df = pd.read_csv('imagenet_20classes_t.csv')
x = df.iloc[:, 0]
...
plt.title('ImageNet1K - 20 classes/task')
plt.legend()
plt.grid(True)
plt.xlim(left=50, right=300)
plt.ylim(bottom=0.1, top=0.7)
plt.tight_layout()
plt.savefig("ImageNet1K-20C.pdf", bbox_inches='tight')
plt.show()
