...
class WGAN(nn.Module):
    def __init__(self, z_size,
        ...
        if dataset == 'MNIST':
            ...
            self.generator = FedCIL_generator.Generator(
                z_size=self.z_size,
                image_size=self.image_size,
                image_channel_size=self.image_channel_size,
        ...
        if dataset == 'CIFAR10':
            self.critic = FedCIL_generator.Critic_CIFAR(
                image_size=self.image_size,
                image_channel_size=self.image_channel_size,
                channel_size=self.c_channel_size,
            )
        ...
        self.D2DCE = Data2DataCrossEntropyLoss(self.num_classes, 0.5, 0.98,
                                               torch.device('cuda:0'))  # default parameter in that paper
    ...
    def _c_loss(self, x, y, classes_so_far, return_g=False, return_aux=False, return_feature=False):
        ...
        fake = self.generator(noise.to(torch.device('cuda:0')))
        dis_output, aux_output, logits, feature_g = self.critic(fake.detach(),
                                                                if_features=True)  # G will not be updated
